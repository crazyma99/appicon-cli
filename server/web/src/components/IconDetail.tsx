...
export function IconDetail({ iconId, onClose, onDelete, onUpdated }: IconDetailProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="bg-surface-1 border border-border rounded-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto"
      ...
      >
            ...
            {sizes.map((size) => {
              ...
              return (
                <div key={size} className="flex flex-col items-center gap-2 shrink-0">
                  <div
                    className="rounded-xl bg-surface-2 border border-border overflow-hidden flex items-center justify-center"
                    style={{ width: displaySize, height: displaySize }}
                  >
                    <img
                      src={`${icon.icons[String(size)]}${icon.icons[String(size)].includes('?') ? '&' : '?'}key=${getApiKey()}`}
                      alt={`${size}px`}
          ...
          {editing ? (
            <div className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={handleSave} className="px-3 py-1.5 bg-accent text-white text-sm rounded-lg cursor-pointer">Save</button>
                <button onClick={() => setEditing(false)} className="px-3 py-1.5 text-text-2 text-sm cursor-pointer">Cancel</button>
              </div>
            </div>
          ) : (
            <div className="space-y-2.5">
              ...
              <div className="pt-3 mt-3 border-t border-border">
                <div className="text-xs font-mono text-text-2 mb-2">Download URLs</div>
                {sizes.map((size) => (
                  <div key={size} className="flex items-center gap-2 text-xs mb-1">
                    <span className="font-mono text-text-2 w-12">{size}px</span>
                    <code className="flex-1 text-text-1 bg-surface-2 rounded px-2 py-1 truncate text-[11px]">
                      {icon.icons[String(size)]}
                    </code>
                    <button
                      onClick={() => navigator.clipboard.writeText(window.location.origin + icon.icons[String(size)])}
                      className="text-accent hover:text-accent-hover cursor-pointer shrink-0"
                      title="Copy URL"
                    >
                      ⎘
                    </button>
        ...
        <div className="px-6 py-4 border-t border-border flex justify-between">
          <button
            ...
            className="px-3 py-1.5 text-danger hover:bg-danger/10 text-sm rounded-lg transition-colors cursor-pointer"
          >
            Delete
          </button>
          <div className="flex gap-2">
            {!editing && (
              <button
                ...
                className="px-3 py-1.5 bg-surface-3 hover:bg-surface-4 text-sm rounded-lg transition-colors cursor-pointer"
              >
                Edit
              </button>
            )}
