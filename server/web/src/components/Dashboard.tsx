...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  if (!stats) {
    return <div className="text-text-2 text-sm animate-pulse">Loading...</div>;
  }

  const statCards = [
    { label: 'Total Icons', value: stats.totalIcons, accent: 'text-accent' },
    { label: 'Icon Files', value: stats.totalFiles, accent: 'text-success' },
    { label: 'Storage', value: formatBytes(stats.totalSize), accent: 'text-warning' },
    { label: 'Categories', value: stats.categories.length, accent: 'text-purple-400' },
  ];
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-text-2 text-sm mt-1">Overview of your icon library</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-surface-1 border border-border rounded-xl p-5">
          ...
          {stats.categories.length === 0 ? (
            <p className="text-text-2 text-sm">No icons yet</p>
          ) : (
        ...
        <div className="bg-surface-1 border border-border rounded-xl p-5">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-sm font-semibold">Recent Icons</h2>
            <button
          ...
          {stats.recentIcons.length === 0 ? (
            <p className="text-text-2 text-sm">No icons yet. Upload your first icon!</p>
          ) : (
              ...
              {stats.recentIcons.map((icon) => (
                <div key={icon.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-2 transition-colors">
                  <div className="w-9 h-9 rounded-lg bg-surface-3 border border-border overflow-hidden flex items-center justify-center">
                    <img
                      src={`/api/icons/${icon.id}/download?size=64`}
                      alt={icon.name}
      ...
      <div className="mt-8 bg-surface-1 border border-border rounded-xl p-5">
        ...
        <div className="bg-surface-2 rounded-lg p-4 font-mono text-xs text-text-1 space-y-1 overflow-x-auto">
          <div className="text-text-2"># Configure appicon CLI to use this server</div>
          <div>
          ...
          <div className="text-text-2 mt-2"># Then search and download</div>
          <div>
